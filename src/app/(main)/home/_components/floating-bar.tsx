...
const searchPlatforms = [
  { name: '抖音', url: 'https://www.douyin.com', path: (key: string) => `jingxuan/search/${key}` },
  { name: 'B站', url: 'https://search.bilibili.com', path: (key: string) => `all?keyword=${key}` },
  { name: '知乎', url: 'https://www.zhihu.com', path: (key: string) => `search?type=content&q=${key}` },
  { name: '百度', url: 'https://www.baidu.com', path: (key: string) => `s?wd=${key}` },
  { name: '维基百科', url: 'https://zh.wikipedia.org', path: (key: string) => `wiki/${key}` },
  { name: '谷歌', url: 'https://www.google.com', path: (key: string) => `search?q=${key}` },
  { name: 'GitHub', url: 'https://github.com', path: (key: string) => `search?q=${key}&type=repositories` },
  { name: 'npm', url: 'https://www.npmjs.com', path: (key: string) => `search?q=${key}` },
];

function FloatingBar({ settings }: { settings: QueryWebsiteSettings[] }) {
  const [version] = useState(() => settings?.find(item => item.name ==='version'));
  const [record] = useState(() => settings?.find(item => item.name ==='record'));
        ...
        <div className="text-sm text-foreground/80 p-2 space-y-2">
          {
            techEnergy.values.map(item => (
              <div key={item.title} className="flex items-center justify-between gap-x-2">
