...
import { createWebsiteSettings, getWebsiteSettings, QueryWebsiteSettings } from '@/lib/api/settings';
...
function SettingsPage() {
  ...
  const { fields: techEnergyValues, append, remove } = useFieldArray({
    control,
  ...
  const onSubmit: SubmitHandler<FormData> = (_data: FormData) => {
    const formData: Record<string, { id?: string, name?: string, title?: string, value: unknown }> = {};
    ...
    for (const key in _data) {
      ...
      if (item?.id) {
        ...
        formData[key] = { name: fieldItem?.field, title: fieldItem?.title , value: _data[key as keyof FormData] };
      }
    }

    if (formData['tech_energy'].value) {
      formData['tech_energy'].value = JSON.stringify(formData['tech_energy'].value);
    }
  ...
  useEffect(() => {
    ...
    const description = data.data.find(item => item.name === 'description')?.content;
    const version = data.data.find(item => item.name === 'version')?.content;
    const publish_date = data.data.find(item => item.name === 'publish_date')?.content;
    const record = data.data.find(item => item.name === 'record')?.content;
    const techStr = data.data.find(item => item.name === 'tech_energy')?.content;
    let tech_energy;
    try {
      tech_energy = techStr ? JSON.parse(techStr) : { description: '', content: [] };
    } catch {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {
              form.map(item => (
                <div key={item.field} className="min-h-16">
                  <div className={cn(
                    'flex gap-x-2',
                    item.field === 'tech_energy' ? 'items-baseline' : 'items-center'
                  )}>
                    <p className="w-24 font-bold text-sm text-right shrink-0">{item.title}：</p>
                    {
                      item.type === 'input' && (
                        <Input
                          ...
                          render={({ field }) => (
                            <Popover>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  <CalendarIcon />
                                  {field.value ? format(field.value, 'yyyy/MM/dd') : <span>请选择建站时间...</span>}
                                </Button>
                              </PopoverTrigger>
                          ...
                          <Controller
                            ...
                            render={({ field, fieldState }) => (
                              <div className="flex flex-col items-start">
                                <Input
                                  {...field}
                                  placeholder="请输入技术力描述"
                                />
                                {
                                  fieldState.error && (
                                    <p className="text-sm font-bold text-red-500 mt-1">{fieldState.error?.message}</p>
                                  )
                                }
                              </div>
                            )}
                              ...
                              techEnergyValues.map((item, idx) => (
                                <div key={item.id} className="flex flex-col gap-y-2">
                                  <div className="flex items-center gap-x-2">
                                    ...
                                    />
                                    <CircleX color='tomato' onClick={() => remove(idx)} />
                                  </div>
                                </div>
                              ))
                            }
                            <Button type="button" size="sm" onClick={() => append({ title: '', value: '' })}>
                              <Plus />
                              <span>添加</span>
                            </Button>
                          </div>
